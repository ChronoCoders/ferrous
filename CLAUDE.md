...
claude --resume 235ed086-df21-4bbc-b53b-61b0eadc1e90
...
- seed1: ~1012 blocks, mining, active, on `b765d12`
- seed4: ~1012 blocks, mining, active, on `b765d12`
- Both nodes on same tip (`332f34c4...` at height 1012, confirmed 2026-03-20 01:01 UTC)
- Git HEAD: `b765d12` (fix: mining read-lock starvation — build_template + solve_template split)
...
- Known open issue: seed4→seed1 outbound handshake intermittently times out (30s). seed1→seed4 outbound always succeeds. Root cause unconfirmed — likely seed1's listener being saturated when mining hard. Not blocking convergence since inbound connections work in both directions.

NEXT: 3-cycle convergence verification test, then Phase 2 (BlockDownloadQueue, RandomX).
...
**[PENDING] Convergence verification (3-cycle test).** Both nodes mining with `--mine`. Procedure:
...
**[PENDING] Asymmetric handshake investigation.** seed4→seed1 outbound intermittently times out. Diagnostic logs deployed at `warn` level in `start_listener`. Check journald on seed1 when seed4 attempts to connect to identify which inbound policy drops it. Run with `RUST_LOG=ferrous_node=debug` on seed1 if warn-level logs are insufficient.
